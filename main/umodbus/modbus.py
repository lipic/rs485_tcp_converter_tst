...
class Modbus(object):
    ...
    def _create_response(self, register_addr: int, reg_type: str, quantity: int):
        """
        Create a response.
        ...
        """
        response: list = list()
        for register_addr in range(register_addr, register_addr + quantity):
            filtered_array = [x for x in list(self._register_dict[reg_type].keys()) if x <= register_addr]
            nearest_mim_reg = max(filtered_array)
            if register_addr in self._register_dict[reg_type].keys():
                for val in self._register_dict[reg_type][register_addr]:
                    response.append(val)
                    break
            elif (len(self._register_dict[reg_type][nearest_mim_reg]) + nearest_mim_reg - 1) >= register_addr:
                offset: int = register_addr - nearest_mim_reg
                response.append(list(self._register_dict[reg_type][nearest_mim_reg])[offset])
            else:
                response.append(0)

        return response

    def _check_register_addr(self, register_addr: int, reg_type: str, quantity: int) -> bool:
        filtered_array = [x for x in list(self._register_dict[reg_type].keys()) if x <= register_addr]
        filtered_array_with_quantity = [x for x in list(self._register_dict[reg_type].keys()) if x <= (register_addr + quantity - 1)]

        if filtered_array and filtered_array_with_quantity:
            nearest_mim_reg = max(filtered_array)
            nearest_mim_reg_with_quantity = max(filtered_array_with_quantity)
            return nearest_mim_reg <= (register_addr + quantity - 1) <= (nearest_mim_reg_with_quantity + len(
                self._register_dict[reg_type][nearest_mim_reg_with_quantity]) - 1)
        else:
            return False
    ...
    def _process_read_access(self, request: Request, reg_type: str) -> None:
        ...
        :type       reg_type:  str
        """
        print(f"Read request with request address: {request.register_addr}, register type: {reg_type}, length: {request.quantity}")
        # if request.register_addr in self._register_dict[reg_type]:

        vals = self._create_response(register_addr=request.register_addr, reg_type=reg_type, quantity=request.quantity)
